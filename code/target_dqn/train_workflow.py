...
from target_dqn.feature.traffic_handler import TrafficHandler
...
def run_episodes(n_episode, env, agent, g_data_truncat, usr_conf, logger):
    for episode in range(n_episode):
        ...
        while not done:
            ...
            frame_no, _obs, score, terminated, truncated, _env_info = traffic_handler.update(env, obs, act)
            count += 1
            ...
            # If the conditions are met, the samples are processed and returned for training
            # 达到条件则进行样本处理并将样本返回进行训练
            if len(collector) % g_data_truncat == 0 and len(collector) > 1:
                collector = sample_process(collector)
                yield collector

            # Can load the latest model on demand
            # 用户可以按需加载最新的模型
